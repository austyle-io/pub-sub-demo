...
import { authenticateWebSocket, type AuthenticatedRequest } from '../middleware/websocket-auth'
...
export class ShareDBService {
  ...
  attachToServer(server: Server): void {
    ...
    server.on('upgrade', (request, socket, head) => {
      authenticateWebSocket(request, socket as any, head, (authenticated) => {
        if (authenticated) {
    ...
    this.wss.on('connection', (ws, req) => {
      const stream = new WebSocketJSONStream(ws) as any
