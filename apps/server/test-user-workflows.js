...
async function testListDocuments() {
  ...
  try {
    ...
    console.log(`   Contains our document: ${result.some(doc => doc.id === documentId)}`);
    
    // Debug output
    if (result.length === 0) {
      console.log('   DEBUG: Empty result, checking accessToken:', !!accessToken);
      console.log('   DEBUG: Result:', JSON.stringify(result));
    } else {
      console.log('   Documents:', result.map(d => ({ id: d.id, title: d.title })));
    }
